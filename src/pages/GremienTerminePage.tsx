...
export default function GremienTerminePage() {
  const [records, setRecords] = useState<GremienTermine[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<GremienTermine | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<GremienTermine | null>(null);
  const [viewingRecord, setViewingRecord] = useState<GremienTermine | null>(null);
  ...
  async function handleCreate(fields: GremienTermine['fields']) {
    await LivingAppsService.createGremienTermineEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: GremienTermine['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateGremienTermineEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteGremienTermineEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      title="Gremien & Termine"
      subtitle={`${records.length} Gremien & Termine im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Gremien & Termine suchen..."
          value={search}
          ...
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('ort')}>
                <span className="inline-flex items-center gap-1">
                  Ort / Videokonferenz-Link
                  {sortKey === 'ort' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('wiederkehrend')}>
                <span className="inline-flex items-center gap-1">
                  Wiederkehrender Termin
                  {sortKey === 'wiederkehrend' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('wiederholungsintervall')}>
                <span className="inline-flex items-center gap-1">
                  Wiederholungsintervall
                  {sortKey === 'wiederholungsintervall' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; setViewingRecord(record); }}>
                ...
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{getPersonenDisplayName(record.fields.teilnehmer)}</span></TableCell>
                ...
                <TableCell><span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${record.fields.wiederkehrend ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'}`}>{record.fields.wiederkehrend ? 'Ja' : 'Nein'}</span></TableCell>
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{record.fields.wiederholungsintervall?.label ?? '—'}</span></TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={11} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Gremien & Termine. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
