...
interface BeteiligungenDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Beteiligungen['fields']) => Promise<void>;
  defaultValues?: Beteiligungen['fields'];
...
export function BeteiligungenDialog({ open, onClose, onSubmit, defaultValues, unternehmenList, personenList, enablePhotoScan = true, enablePhotoLocation = true }: BeteiligungenDialogProps) {
  const [fields, setFields] = useState<Partial<Beteiligungen['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'beteiligungen');
      await onSubmit(clean as Beteiligungen['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="beteiligte_personen" entity="Personen">\n${JSON.stringify(personenList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      if (usePersonalInfo) {
        try {
      ...
      const schema = `{\n  "unternehmen": string | null, // Display name from Unternehmen (see <available-records>)\n  "beteiligte_personen": string | null, // Display name from Personen (see <available-records>)\n  "beteiligungsart": LookupValue | null, // Beteiligungsart (select one key: "direkt" | "indirekt" | "fonds" | "wandeldarlehen" | "sonstige") mapping: direkt=Direkt, indirekt=Indirekt, fonds=Über Fonds, wandeldarlehen=Wandeldarlehen, sonstige=Sonstige\n  "anteil_prozent": number | null, // Anteil (%)\n  "investiertes_kapital": number | null, // Investiertes Kapital (EUR)\n  "aktueller_wert": number | null, // Aktueller Wert (EUR)\n  "eintrittsdatum": string | null, // YYYY-MM-DD\n  "exitdatum": string | null, // YYYY-MM-DD\n  "bewertung": string | null, // Bewertung / Kommentar\n  "notizen_beteiligung": string | null, // Notizen\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["unternehmen", "beteiligte_personen"]);
        ...
        const beteiligte_personenName = raw['beteiligte_personen'] as string | null;
        if (beteiligte_personenName) {
          const beteiligte_personenMatch = personenList.find(r => matchName(beteiligte_personenName!, [[r.fields.vorname ?? '', r.fields.nachname ?? ''].filter(Boolean).join(' ')]));
          if (beteiligte_personenMatch) merged['beteiligte_personen'] = createRecordUrl(APP_IDS.PERSONEN, beteiligte_personenMatch.record_id);
        }
        return merged as Partial<Beteiligungen['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Beteiligungen bearbeiten' : 'Beteiligungen hinzufügen';
        ...
        <form onSubmit={handleSubmit} className="flex flex-1 flex-col min-h-0">
          ...
          <div className="space-y-1.5">
            ...
            <Combobox
              ...
              value={extractRecordId(fields.beteiligte_personen)}
              onChange={id => setFields(f => ({ ...f, beteiligte_personen: id ? createRecordUrl(APP_IDS.PERSONEN, id) : undefined }))}
              placeholder="Auswählen..."
              searchPlaceholder="Suchen…"
              emptyText="Kein Treffer"
            />
