...
export function PersonenDialog({ open, onClose, onSubmit, defaultValues, unternehmenList, enablePhotoScan = true, enablePhotoLocation = true }: PersonenDialogProps) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "unternehmen": string | null, // Display name from Unternehmen (see <available-records>)\n  "vorname": string | null, // Vorname\n  "nachname": string | null, // Nachname\n  "email": string | null, // E-Mail\n  "telefon": string | null, // Telefon\n  "rolle": LookupValue | null, // Rolle/Funktion (select one key: "ansprechpartner" | "beirat" | "geschaeftsfuehrer" | "investor" | "berater" | "sonstiges") mapping: ansprechpartner=Ansprechpartner, beirat=Beirat, geschaeftsfuehrer=Geschäftsführer, investor=Investor, berater=Berater, sonstiges=Sonstiges\n  "linkedin": string | null, // LinkedIn/Xing-Profil\n  "notizen_person": string | null, // Notizen\n}`;
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[92vh] flex flex-col overflow-hidden p-0 gap-0">
        ...
        <form onSubmit={handleSubmit} className="flex flex-1 flex-col min-h-0">
          ...
          <div className="space-y-1.5">
            <Label htmlFor="rolle">Rolle/Funktion</Label>
            <Select
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="ansprechpartner">Ansprechpartner</SelectItem>
          ...
          <div className="space-y-1.5">
            <Label htmlFor="linkedin">LinkedIn/Xing-Profil</Label>
            <Input
              id="linkedin"
          ...
          <div className="space-y-1.5">
            <Label htmlFor="notizen_person">Notizen</Label>
            <Textarea
              id="notizen_person"
              value={fields.notizen_person ?? ''}
              onChange={e => setFields(f => ({ ...f, notizen_person: e.target.value }))}
              rows={3}
            />
          </div>
          </div>
